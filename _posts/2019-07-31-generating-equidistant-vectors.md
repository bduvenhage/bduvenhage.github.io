...
The ratios between consecutive Fibonacci numbers approach the golden ratio. Also, an alternative for expressing the Fibonacci sequence is $$F\left(n\right)=\frac{\varphi^{n} - (1-\varphi)^{n}}{\sqrt{5}}$$. #mindblown 🤯. The spiral walk discussed here is therefore often referred to as a spherical Fibonacci lattice or a Fibonacci spiral sphere.

The method presented here I originally implemented for a paper on [Numerical Verification of Bidirectional Reflectance Distribution Functions for Physical Plausibility](https://dl.acm.org/citation.cfm?id=2513499). The PDF of a pre-print of the paper is also available from [ResearchGate](https://www.researchgate.net/publication/259885429_Numerical_Verification_of_Bidirectional_Reflectance_Distribution_Functions_for_Physical_Plausibility) and via my [Google Scholar page](https://scholar.google.com/citations?user=jqhH0o4AAAAJ). The paper also discusses an alternative method based on subdivision of a 20-sided regular icosahedron.
...
## Summary
Full [source](https://github.com/bduvenhage/Bits-O-Cpp/tree/master/geometry) with execution timing
